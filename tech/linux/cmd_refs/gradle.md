...
###### Clear gradle cache (always stop the daemon after manually clearing the cache)
```bash
rm -rf ~/.gradle/caches/
./gradlew --stop
```

###### Clear gradle daemon (resolves error `Could not create SecurityManager`)
```bash
rm -rf ~/.gradle/daemon/
```

###### Quiet mode
`gradle -q` suppresses gradle's log messages, so that only the output of the tasks is shown. This keep the output a little clearer.
