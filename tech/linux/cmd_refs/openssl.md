# Misc. generic cryptography related Linux OpenSSL commands
...
###### Generate an RSA private key (PEM format output)
```bash
openssl genrsa -out private.key -pubout > public.pem
```

###### Generate RSA public key from private key (PEM format private key)
```bash
openssl rsa -in private.key -pubout > public.pem
```

###### Generate RSA public key from private key (DER format private key)
```bash
openssl rsa -in private.key -inform DER -pubout > public.pem
```

###### Convert PEM encoded private key to PKCS#8 DER encoding
```bash
openssl pkcs8 -topk8 -inform PEM -outform DER -in filename.pem -out filename.key -nocrypt
```

###### The correct way to calculate the hash of a key
```bash
openssl pkey -pubin -in public.pem -pubout | openssl sha256 -c
```
Why? If you just simply use `openssl sha256 public.pem`, you will get different results if the two keys have different line-endings even if they are the same. Use `file <filename>` to check the line-endings.

If you want Yes/No output:
```bash
out1=$(openssl pkey -pubin -in public.pem -pubout | openssl sha256 -c)
out1=$(openssl pkey -pubin -in public.pem -pubout | openssl sha256 -c)
test "$out1" = "$out2" && echo Yes || echo No
```

###### Encrypt an AES key with an RSA public key (PEM format)
```bash
openssl rsautl -encrypt -inkey public.pem -pubin -in aes.key -out aes.key.enc
```

###### Decrypt an AES key with an RSA private key (DER format private key)
```bash
openssl rsautl -decrypt -inkey private.key -keyform DER -in aes.key.enc -out aes.key
```
...
###### Inspect DER encoded key
```bash
openssl asn1parse -inform DER -in private.der.key
```
...
COMMAND SUMMARY
       ...
       both cases, the output goes to stdout and nothing is printed to stderr.  Additional command line arguments are always ignored.  Since for each cipher there is a command of the same name, this provides
       an easy way for shell scripts to test for the availability of ciphers in the openssl program.  (no-XXX is not able to detect pseudo-commands such as quit, list, or no-XXX itself.)
   ...
   Standard Commands
       ...
       s_server
           ...
           functionality but internally uses mostly all functionality of the OpenSSL ssl library.  It provides both an own command line oriented protocol for testing SSL functions and a simple HTTP response
...
SEE ALSO
       ...
       nseq(1), ocsp(1), passwd(1), pkcs12(1), pkcs7(1), pkcs8(1), pkey(1), pkeyparam(1), pkeyutl(1), prime(1), rand(1), rehash(1), req(1), rsa(1), rsautl(1), s_client(1), s_server(1), s_time(1), sess_id(1),
...
1.1.1l                                                                                             2021-11-24                                                                                     OPENSSL(1SSL)
